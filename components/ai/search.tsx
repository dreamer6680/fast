...
function SearchAIInput(props: FormHTMLAttributes<HTMLFormElement>) {
  ...
  return (
    ...
    >
      <Input
        value={input}
        placeholder={isLoading ? t('ai.chat.placeholderLoading') : t('ai.chat.placeholder')}
...
// We'll use translations for role names now
...
function Message({ message }: { message: Message }) {
  ...
  const getRoleName = (role: string) => {
    if (role === 'user') return t('ai.chat.you');
    if (role === 'assistant') return t('ai.chat.assistant');
    return 'unknown';
  };
        ...
        <div className="mt-2 flex flex-row flex-wrap items-center gap-1">
          {links.map((item, i) => (
            ...
            >
              ...
              <p className="text-fd-muted-foreground">{t('ai.chat.reference')} {item.label}</p>
            </Link>
          ))}
        </div>
      ) : null}
...
function Content() {
  const { t } = useTranslations();
  const chat = useChat({
    api: '/api/chat/custom', // 使用我们的自定义 API
    id: 'search',
    streamProtocol: 'data',
        ...
        <div className="rounded-xl overflow-hidden border shadow-lg bg-fd-popover text-fd-popover-foreground">
          ...
          <div className="flex gap-2 items-center text-fd-muted-foreground px-3 py-1.5">
            <DialogTitle className="text-xs flex-1">
              {t('ai.chat.poweredBy')}. {t('ai.chat.disclaimer')}
            </DialogTitle>
            <DialogClose
              aria-label="Close"
              tabIndex={-1}
