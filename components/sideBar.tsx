  ...
  const check = (item: PageTree.Item | PageTree.Folder): boolean => {
    if ("children" in item) {
      return item.children
        .filter((child): child is PageTree.Item | PageTree.Folder =>
          "url" in child || "children" in child
        )
        .some(check);
    }
...
const CustomFolder: FC<{ item: PageTree.Folder; level: number; children: ReactNode }> = ({
  ...
  return (
    <SidebarFolder defaultOpen={shouldExpand} className="bg-blue hover:cursor-pointer">
      ...
      <SidebarFolderContent className="bg-blue hover:cursor-pointer">{children}</SidebarFolderContent>
    </SidebarFolder>
  );
};

const CustomSeparator: FC<{ item: PageTree.Separator }> = ({ item }) => (
  <div className="bg-blue text-xs text-gray-400 px-2 py-1 uppercase tracking-wide">
    {item.name}
  </div>
);
